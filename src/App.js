...
const LoginPage = ({ setPage, setUser }) => {
  ...
  const login = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      if (nickname === "admin" && pass === "admin123") {
        setUser({ nickname: "Admin", role: "ADMIN", anonymousId: "ADM-001" });
        setPage("admin-dash"); setLoading(false); return;
      }
      const users = getUsers();
      const found = users.find(u => u.nickname.toLowerCase() === nickname.toLowerCase() && u.password === pass);
      if (!found) { setError("Nesprávná přezdívka nebo heslo."); setLoading(false); return; }
      setUser(found);
      setPage(found.role === "SELLER" ? "seller-dash" : "buyer-dash");
      setLoading(false);
    }, 800);
  };
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 48 }}>
        <div style={{ width: "100%", maxWidth: 380 }}>
          ...
          <Card style={{ padding: 26 }}>
            ...
            <p style={{ textAlign: "center", marginTop: 6, fontSize: 11, color: "#ddd" }}>Admin: přezdívka <strong>admin</strong>, heslo <strong>admin123</strong></p>
          </Card>
        </div>
      </div>
    </div>
...
const InsolvencyCheck = ({ firstName, lastName, onResult }) => {
  ...
  const check = async () => {
    ...
    try {
      ...
      if (hasActive) { setStatus("insolvent"); setResult(data.items[0]); onResult && onResult(false); }
  ...
  if (status === "idle") return (
    <button onClick={check} style={{ display: "flex", alignItems: "center", gap: 8, background: "#f7f4ef", border: "1.5px solid #e0dbd4", borderRadius: 10, padding: "10px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", color: "#1a1a1a" }}>
  ...
  if (status === "ok") return (
    <div style={{ background: "#d1fae5", border: "1px solid #6ee7b7", borderRadius: 10, padding: "10px 16px", fontSize: 13, color: "#065f46", fontWeight: 600 }}>
      ✅ Žádný insolvenční záznam nalezen
    </div>
  );
  if (status === "insolvent") return (
    <div style={{ background: "#fee2e2", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: "#991b1b" }}>
      <div style={{ fontWeight: 700, marginBottom: 4 }}>⛔ Aktivní insolvence nalezena!</div>
      <div>Číslo jednací: {result?.cisloJednaci || "–"}</div>
    </div>
  );
  return (
    <div style={{ background: "#fef3c7", border: "1px solid #fde68a", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: "#92400e" }}>
      <div style={{ fontWeight: 700, marginBottom: 4 }}>⚠️ Automatická kontrola nedostupná (CORS)</div>
      <div style={{ marginBottom: 8 }}>Zkontrolujte ručně na webu ISIR:</div>
    ...
    <div style={{ maxWidth: 720, margin: "40px auto", padding: "0 24px" }}>
      ...
      {step !== "success" && step !== "processing" && (
        <div style={{ display: "flex", alignItems: "center", gap: 0, marginBottom: 32 }}>
          {[{ n: 1, label: "Osobní údaje", s: "personal" }, { n: 2, label: "Platba", s: "payment" }].map((st, i) => {
            const active = step === st.s;
            const done = step === "payment" && st.s === "personal";
            return (
              <div key={st.n} style={{ display: "flex", alignItems: "center", flex: i < 1 ? 1 : undefined }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: done ? "#059669" : active ? "#1a1a1a" : "#e0dbd4", color: done || active ? "#fff" : "#999", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700 }}>{done ? "✓" : st.n}</div>
                  <span style={{ fontSize: 13, fontWeight: active ? 700 : 400, color: active ? "#1a1a1a" : "#999" }}>{st.label}</span>
                </div>
                {i < 1 && <div style={{ flex: 1, height: 1, background: "#e0dbd4", margin: "0 12px" }} />}
              </div>
            );
          })}
        </div>
        ...
        <div style={{ textAlign: "center", padding: "56px 0" }}>
          ...
          <Card style={{ padding: 26, marginBottom: 24 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, textAlign: "left" }}>
              <div><div style={{ fontSize: 10, color: "#999", fontWeight: 600, marginBottom: 7 }}>PRODÁVAJÍCÍ — ODHALENO</div><div style={{ fontWeight: 800, fontSize: 17, marginBottom: 3 }}>Kateřina Novotná</div><div style={{ color: "#059669", fontSize: 13 }}>📧 k.novotna@email.cz</div><div style={{ color: "#059669", fontSize: 13 }}>📞 +420 732 456 789</div></div>
              <div><div style={{ fontSize: 10, color: "#999", fontWeight: 600, marginBottom: 7 }}>VY — ODHALENO</div><div style={{ fontWeight: 800, fontSize: 17, marginBottom: 3 }}>{form.firstName} {form.lastName}</div><div style={{ color: "#059669", fontSize: 13 }}>📞 {form.phone}</div></div>
            </div>
          </Card>
      ...
      {step === "personal" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 24 }}>
          <div>
            <h2 style={{ ...D, fontSize: 24, fontWeight: 800, marginBottom: 6 }}>Osobní údaje</h2>
            <p style={{ color: "#888", fontSize: 13, marginBottom: 24 }}>Před rezervací musíme ověřit vaši totožnost a zkontrolovat insolvenční rejstřík.</p>
            <Card style={{ padding: 26 }}>
              ...
              <Btn full disabled={insolvencyOk === false} onClick={() => { if (validatePersonal()) setStep("payment"); }}>
                Pokračovat k platbě →
              </Btn>
            </Card>
          </div>
          <Card style={{ padding: 22, alignSelf: "start" }}>
            ...
            <div style={{ fontSize: 12, color: "#888", marginBottom: 14 }}>📍 {l.district}, Praha</div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 800, borderTop: "1px solid #f0ede8", paddingTop: 14 }}><span>Poplatek</span><span style={{ ...D }}>50 000 Kč</span></div>
            <div style={{ marginTop: 12, background: "#f0fdf4", borderRadius: 9, padding: 10, fontSize: 11, color: "#065f46" }}>✓ Identity odhaleny<br />✓ Přímá komunikace<br />✓ 48h blokace</div>
          </Card>
        </div>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 24 }}>
          <Card style={{ padding: 26 }}>
            ...
            <div style={{ background: "#fef3c7", border: "1px solid #fde68a", borderRadius: 10, padding: "10px 14px", marginBottom: 18, fontSize: 12, color: "#92400e" }}>
              ⚠️ Po odeslání platby klikněte na tlačítko níže.
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <Btn variant="ghost" onClick={() => setStep("personal")}>← Zpět</Btn>
          ...
          <Card style={{ padding: 22, alignSelf: "start" }}>
            ...
            <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>👤 {form.firstName} {form.lastName}</div>
            <div style={{ fontSize: 12, color: "#059669", fontWeight: 600 }}>✓ Osobní údaje ověřeny</div>
          </Card>
        </div>
      )}
    ...
    <div>
      ...
      <div style={{ background: "#f7f4ef", borderTop: "1px solid #ede9e3", borderBottom: "1px solid #ede9e3", padding: "28px 32px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
          {[{ icon: "💰", title: "O 50 % nižší provize", desc: "Ušetříte stovky tisíc. Cena jasně daná předem." }, { icon: "📊", title: "Plná transparentnost", desc: "Máte přehled o každém kroku prodeje." }, { icon: "✅", title: "Jen prověření kupující", desc: "Jednáte pouze s reálnými zájemci." }].map(f => (
            <div key={f.title} style={{ textAlign: "center", padding: "24px 16px" }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>{f.icon}</div>
              <h3 style={{ ...D, fontSize: 18, fontWeight: 700, marginBottom: 8 }}>{f.title}</h3>
              ...
              <div style={{ padding: "0 24px 24px" }}>
                ...
                {vl ? (
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 24 }}>
                    <div><div style={{ color: "#888", fontSize: 11, fontWeight: 600, marginBottom: 8 }}>ODHADOVANÁ CENA</div><div style={{ ...D, color: "#C9A84C", fontSize: 26, fontWeight: 800 }}>{fmt(vr.value)}</div><div style={{ color: "#666", fontSize: 12 }}>Rozsah: {fmt(vr.low)} – {fmt(vr.high)}</div></div>
                    ...
                    <div><div style={{ color: "#888", fontSize: 11, fontWeight: 600, marginBottom: 8 }}>TREND TRHU</div><div style={{ color: "#34d399", fontSize: 20, fontWeight: 700 }}>↗ Rostoucí</div></div>
                  </div>
                )}
              </div>
            )}
...
const ListingCard = ({ l, setPage }) => (
  <Card onClick={() => setPage("listing-" + l.id)} style={{ padding: 0 }}>
    ...
    <div style={{ padding: 18 }}>
      ...
      <div style={{ fontSize: 11, color: "#C9A84C", fontWeight: 600, marginTop: 2 }}>✦ AI: {fmt(l.aiValuation)}</div>
    </div>
  </Card>
);
...
const ListingsPage = ({ setPage }) => {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <p style={{ color: "#888", fontSize: 14, marginBottom: 24 }}>{LISTINGS.length} nemovitostí v Praze</p>
      <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
        <input placeholder="🔍  Hledat…" value={search} onChange={e => setSearch(e.target.value)}
          style={{ flex: 1, minWidth: 200, padding: "10px 16px", borderRadius: 12, border: "1.5px solid #e0dbd4", fontSize: 14, fontFamily: "'DM Sans', sans-serif" }} />
...
const ListingDetail = ({ id, setPage, user }) => {
  ...
  const getAI = () => {
    setAil(true);
    setTimeout(() => {
      setAi({ suggested: Math.round(l.aiValuation * 0.96), winProb: 74, tip: l.daysOnMarket > 20 ? `Inzerováno ${l.daysOnMarket} dní. Prodávající je pravděpodobně flexibilní.` : `Čerstvá inzerce. Doporučujeme nabídku blízkou ceně.` });
      setAil(false);
    }, 1200);
  };
          ...
          <Card style={{ padding: 26 }}>
            ...
            {!showOffer ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                ...
                <Btn variant="ghost" full onClick={() => { if (!user) { setPage("login"); return; } setPage("reserve-" + l.id); }}>Rezervovat (50 000 Kč)</Btn>
              </div>
            ) : submitted ? (
              <div style={{ textAlign: "center", padding: "18px 0" }}>
                <div style={{ fontSize: 36, marginBottom: 10 }}>🎉</div>
                <div style={{ fontWeight: 700 }}>Nabídka odeslána!</div>
              </div>
            ) : (
              <div>
                {ail ? <div style={{ textAlign: "center", padding: "12px", color: "#888", fontSize: 13 }}>AI analyzuje…</div> : ai && (
                  <div style={{ background: "#1a1a1a", borderRadius: 12, padding: 14, marginBottom: 14 }}>
                    ...
                    <p style={{ color: "#aaa", fontSize: 12, lineHeight: 1.6, margin: 0 }}>{ai.tip}</p>
                  </div>
                )}
            ...
            <div style={{ flex: 1 }}>
              ...
              {o.status === "ACCEPTED" && (
                <div style={{ padding: "9px 12px", background: "#f0fdf4", borderRadius: 9, fontSize: 12, color: "#065f46" }}>
                  🎉 Přijato! <button onClick={() => setPage("reserve-" + o.listing.id)} style={{ background: "none", border: "none", color: "#059669", fontWeight: 700, cursor: "pointer", textDecoration: "underline", fontFamily: "'DM Sans', sans-serif", fontSize: 12 }}>Zaplatit rezervaci →</button>
                </div>
              )}
            </div>
          </div>
      ...
      {tab === "reservations" && (
        <Card style={{ padding: 26 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
            <div style={{ background: "#f0fdf4", borderRadius: 12, padding: 16 }}><div style={{ fontSize: 10, color: "#888", fontWeight: 600, marginBottom: 7 }}>PRODÁVAJÍCÍ</div><div style={{ fontWeight: 800, color: "#059669" }}>Kateřina Novotná</div><div style={{ fontSize: 12, color: "#059669" }}>📧 k.novotna@email.cz</div></div>
            <div style={{ background: "#f7f4ef", borderRadius: 12, padding: 16 }}><div style={{ fontSize: 10, color: "#888", fontWeight: 600, marginBottom: 7 }}>ZAPLACENO</div><div style={{ ...D, fontSize: 22, fontWeight: 800 }}>50 000 Kč</div></div>
          </div>
        </Card>
      )}
    </div>
