...
const AdminDash = () => {
  ...
  const approve = (id) => {
    setListingStatuses(s => ({ ...s, [id]: "active" }));
    showToast("✓ Inzerát schválen a zveřejněn");
  };
  const reject = (id) => {
    setListingStatuses(s => ({ ...s, [id]: "rejected" }));
    setShowReject(s => ({ ...s, [id]: false }));
    showToast("Inzerát zamítnut", "#991b1b");
  };
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 28 }}>
        {[
          ...
          { label: "Uživatelé", value: users.length, icon: "👤", color: "#3b82f6" },
          { label: "Rezervace", value: ALL_ACTIVITY.filter(a => a.type === "reservation").length, icon: "🔑", color: "#7c3aed" },
        ].map(s => (
          <Card key={s.label} style={{ padding: 18 }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 22, borderBottom: "1px solid #e0dbd4" }}>
        {[{ id: "overview", label: "Aktivita" }, { id: "listings", label: "Inzeráty" + (pendingCount > 0 ? " (" + pendingCount + " čeká)" : "") }, { id: "users", label: "Uživatelé" }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: "9px 18px", background: "none", border: "none", borderBottom: tab === t.id ? "2px solid #1a1a1a" : "2px solid transparent", color: tab === t.id ? "#1a1a1a" : "#888", fontWeight: tab === t.id ? 700 : 500, fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", marginBottom: -1 }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === "listings" && (
        <div>
                  ...
                  <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>{l.title}</div>
                          <div style={{ fontSize: 12, color: "#888" }}>📍 {l.district} · {l.seller?.anonymousId} · {fmtFull(l.askingPrice)}</div>
                        </div>
                        <Badge color="amber">Čeká na schválení</Badge>
                      </div>
                      {/* Validační checklist */}
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                        {[["Katastr", true], ["Fotografie", (l.imgs || []).length > 0], ["Duplicity", true], ["Cena", l.askingPrice < 50000000]].map(([label, ok]) => (
                          <span key={label} style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 99, background: ok ? "#d1fae5" : "#fee2e2", color: ok ? "#065f46" : "#991b1b" }}>
                            {ok ? "✓" : "⚠"} {label}
                          </span>
                        ))}
                      </div>
          ...
          {/* Všechny inzeráty */}
          <div style={{ fontSize: 11, fontWeight: 700, color: "#999", marginBottom: 12, letterSpacing: "0.05em" }}>VŠECHNY INZERÁTY</div>
          {listings.map(l => {
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "#888" }}>📍 {l.district} · {l.seller?.anonymousId} · {fmt(l.askingPrice)}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <Badge color={status === "active" ? "green" : status === "pending" ? "amber" : "red"}>
                      {status === "active" ? "Aktivní" : status === "pending" ? "Čeká" : "Zamítnuto"}
                    </Badge>
                    {status === "pending" && <Btn small onClick={() => approve(l.id)}>Schválit</Btn>}
                    {status === "rejected" && rejectReason[l.id] && <span style={{ fontSize: 11, color: "#991b1b" }}>{rejectReason[l.id]}</span>}
                  </div>
                </div>
              </Card>
            );
