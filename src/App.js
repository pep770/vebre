...
const NewListingPage = ({ setPage, user }) => {
  const [form, setForm] = useState({ title: "", address: "", district: "", propertyType: "APARTMENT", bedrooms: 2, bathrooms: 1, internalSize: "", yearBuilt: "", askingPrice: "", hasGarage: false, hasBalcony: false, hasGarden: false, hasPool: false, description: "", features: "" });
  ...
  const save = () => {
    if (!form.title || !form.address || !form.askingPrice || !form.internalSize) { setError("Vyplňte název, adresu, plochu a cenu."); return; }
    if (imgs.length === 0) { setError("Nahrajte alespoň jednu fotku."); return; }
    setSaving(true);
    setTimeout(() => {
      ...
      const newListing = {
        ...form,
        id: "l" + Date.now(),
        bedrooms: +form.bedrooms, bathrooms: +form.bathrooms, internalSize: +form.internalSize,
        yearBuilt: +form.yearBuilt, askingPrice: +form.askingPrice, reservationFee: 50000,
        aiValuation: Math.round(+form.askingPrice * 1.03), aiConfidence: 0.85,
        aiRange: { low: Math.round(+form.askingPrice * 0.93), high: Math.round(+form.askingPrice * 1.1) },
        ...
        aiFactors: [{ factor: "Nová nemovitost", impact: "positive", score: 80 }],
        comparables: [],
        city: "Praha",
      };
  ...
  return (
    <div style={{ maxWidth: 760, margin: "40px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("seller-dash")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 24, fontFamily: "'DM Sans', sans-serif" }}>← Zpět</button>
      <h1 style={{ ...D, fontSize: 28, fontWeight: 800, marginBottom: 6 }}>Nový inzerát</h1>
      <p style={{ color: "#888", fontSize: 13, marginBottom: 28 }}>Vyplňte údaje a nahrajte fotky nemovitosti.</p>

      <Card style={{ padding: 28, marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#999", marginBottom: 14, letterSpacing: "0.05em" }}>FOTKY NEMOVITOSTI</div>
        <ImageUploader listingId={"new_" + Date.now()} currentImgs={[]} onImagesUpdated={setImgs} />
      </Card>

      <Card style={{ padding: 28, marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#999", marginBottom: 14, letterSpacing: "0.05em" }}>ZÁKLADNÍ ÚDAJE</div>
        <Input label="NÁZEV INZERÁTU" value={form.title} onChange={setF("title")} placeholder="Např. Prostorný byt 3+kk v centru Prahy" />
        <Input label="ADRESA" value={form.address} onChange={setF("address")} placeholder="Ulice, číslo, Praha X - Čtvrť" />
        <Input label="ČTVRŤ" value={form.district} onChange={setF("district")} placeholder="Např. Vinohrady" />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#999", display: "block", marginBottom: 5, letterSpacing: "0.05em" }}>TYP</label>
            <select value={form.propertyType} onChange={setF("propertyType")} style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #e0dbd4", fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>
              <option value="APARTMENT">Byt</option>
              <option value="HOUSE">Dům</option>
              <option value="OFFICE">Kancelář</option>
            </select>
          </div>
          <Input label="POČET POKOJŮ" type="number" value={form.bedrooms} onChange={setF("bedrooms")} />
          <Input label="KOUPELNY" type="number" value={form.bathrooms} onChange={setF("bathrooms")} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <Input label="PLOCHA (M²)" type="number" value={form.internalSize} onChange={setF("internalSize")} placeholder="80" />
          <Input label="ROK VÝSTAVBY" type="number" value={form.yearBuilt} onChange={setF("yearBuilt")} placeholder="2010" />
          <Input label="POŽADOVANÁ CENA (KČ)" type="number" value={form.askingPrice} onChange={setF("askingPrice")} placeholder="8000000" />
        </div>
      </Card>

      <Card style={{ padding: 28, marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#999", marginBottom: 14, letterSpacing: "0.05em" }}>VYBAVENÍ</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          {[{ key: "hasGarage", label: "🚗 Garáž" }, { key: "hasBalcony", label: "🏙 Balkón" }, { key: "hasGarden", label: "🌿 Zahrada" }, { key: "hasPool", label: "🏊 Bazén" }].map(item => (
            <label key={item.key} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", padding: "10px 14px", background: form[item.key] ? "#f0fdf4" : "#f7f4ef", borderRadius: 10, border: `1.5px solid ${form[item.key] ? "#6ee7b7" : "#e0dbd4"}` }}>
              <input type="checkbox" checked={form[item.key]} onChange={setF(item.key)} style={{ width: 16, height: 16 }} />
              <span style={{ fontSize: 13, fontWeight: 600 }}>{item.label}</span>
            </label>
          ))}
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={{ fontSize: 11, fontWeight: 700, color: "#999", display: "block", marginBottom: 5, letterSpacing: "0.05em" }}>POPIS NEMOVITOSTI</label>
          <textarea value={form.description} onChange={setF("description")} rows={4} placeholder="Popište nemovitost..." style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #e0dbd4", fontSize: 13, fontFamily: "'DM Sans', sans-serif", boxSizing: "border-box", resize: "vertical" }} />
        </div>
        <Input label="VLASTNOSTI (oddělené čárkou)" value={form.features} onChange={setF("features")} placeholder="Terasa, Výhled na park, Novostavba" />
      </Card>

      {error && <div style={{ background: "#fee2e2", color: "#991b1b", borderRadius: 9, padding: "12px 16px", fontSize: 13, marginBottom: 16 }}>⚠️ {error}</div>}
      <Btn full onClick={save} disabled={saving}>{saving ? "Ukládám…" : "Zveřejnit inzerát →"}</Btn>
