    ...
    <div style={{ maxWidth: 600, margin: "60px auto", padding: "0 24px", textAlign: "center" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #ede9e3", borderRadius: 14, padding: "16px 20px", marginBottom: 24, textAlign: "left" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "#f7f4ef", borderRadius: 10 }}>
          ...
          <a href="#" onClick={e => e.preventDefault()} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#4285F4", color: "#fff", padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        const vebreRate = 0.02;
        const agencyRate = 0.04;
        const vebreFee = Math.round(price * 1e6 * vebreRate);
        const agencyFee = Math.round(price * 1e6 * agencyRate);
        const savings = agencyFee - vebreFee;
        return (
          <div style={{ background: "#f7f4ef", borderTop: "1px solid #ede9e3", padding: "56px 32px" }}>
                ...
                <div style={{ marginBottom: 14, padding: "12px 14px", background: "#f7f4ef", borderRadius: 11, border: "1px solid #e0dbd4" }}>
                  ...
                  <div style={{ position: "relative", height: 8, background: "#e0dbd4", borderRadius: 99, marginBottom: 6 }}>
                    ...
                    {competingOffers.map((o, i) => (
                      <div key={i} style={{ position: "absolute", top: -3, left: `${o.pct * 100 - 91}%`, width: 14, height: 14, borderRadius: "50%", background: "#fff", border: "2.5px solid #888", transform: "translateX(-50%)" }} title={o.label} />
                    ))}
                    <div style={{ position: "absolute", top: -3, left: `${(amount / l.askingPrice) * 100 - 91}%`, width: 14, height: 14, borderRadius: "50%", background: "#1a1a1a", border: "2.5px solid #C9A84C", transform: "translateX(-50%)" }} title="Vaše nabídka" />
                  </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {PORTALS.map(p => (
              <div key={p.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", background: "#f7f4ef", borderRadius: 12 }}>
                ...
                {p.status === "active" && (
                  <a href="#" onClick={e => e.preventDefault()} style={{ fontSize: 12, color: "#3b82f6", fontWeight: 600, textDecoration: "none" }}>Zobrazit inzerát →</a>
                )}
