...
const SellerDash = ({ setPage, user }) => {
  ...
  const pendingViewings = myViewings.filter(v => v.status === "PENDING");

  const confirmViewing = (viewingId) => {
    const updated = getViewings().map(v => v.id === viewingId ? { ...v, status: "CONFIRMED" } : v);
    saveViewings(updated);
    window.location.reload(); // jednoduché obnovení pro demo
  };
  const cancelViewing = (viewingId) => {
    const updated = getViewings().map(v => v.id === viewingId ? { ...v, status: "CANCELLED" } : v);
    saveViewings(updated);
    ...
    <div style={{ maxWidth: 1050, margin: "0 auto", padding: "28px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: "#888", fontSize: 13 }}>Správa inzerátů a nabídek</p>
        </div>
      ...
      {pendingViewings.length > 0 && (
        <div style={{ background: "#fef3c7", border: "1px solid #fde68a", borderRadius: 14, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 13, color: "#92400e", fontWeight: 600 }}>
            📅 Máte <strong>{pendingViewings.length}</strong> nové žádosti o prohlídku čekající na potvrzení
          </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 22, borderBottom: "1px solid #e0dbd4" }}>
        {[{ id: "listings", label: "Moje inzeráty" }, { id: "viewings", label: `📅 Prohlídky${pendingViewings.length > 0 ? ` (${pendingViewings.length} nové)` : ""}` }, { id: "offers", label: "Příchozí nabídky" }].map(t => (
          ...
          <Card key={l.id} style={{ padding: 0, display: "flex", overflow: "hidden", marginBottom: 14 }}>
            ...
            <div style={{ padding: "18px 22px", flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                ...
                <span style={{ fontSize: 12, color: "#888" }}>📅 {getViewings().filter(v => v.listingId === l.id).length} prohlídek</span>
              </div>
            </div>
      ...
      {tab === "viewings" && (
        myViewings.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>📅</div>
            <div style={{ fontWeight: 600, marginBottom: 8 }}>Zatím žádné žádosti o prohlídku</div>
            <p style={{ color: "#888" }}>Nastavte dostupné termíny a kupující si budou moci rezervovat prohlídky.</p>
          </div>
        ) : (
            ...
            {myViewings.map(v => (
              <Card key={v.id} style={{ padding: 22 }}>
                <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  <img src={v.listingImg} alt={v.listingTitle} style={{ width: 72, height: 54, objectFit: "cover", borderRadius: 8, flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 2 }}>{v.listingTitle}</div>
                        <div style={{ fontSize: 12, color: "#888" }}>Kupující: {v.buyerAnonymousId} · {new Date(v.createdAt).toLocaleDateString("cs-CZ")}</div>
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                      <span style={{ background: "#f7f4ef", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600 }}>📅 {v.slot.date}</span>
                      <span style={{ background: "#f7f4ef", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600 }}>🕐 {v.slot.time}</span>
                      <span style={{ background: "#f7f4ef", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600 }}>📞 {v.form?.phone}</span>
                    </div>
                    {v.form?.note && <div style={{ background: "#f7f4ef", borderRadius: 9, padding: "8px 12px", fontSize: 12, color: "#666", marginBottom: 10 }}>💬 {v.form.note}</div>}
                    {v.status === "PENDING" && (
                      <div style={{ display: "flex", gap: 8 }}>
                        <Btn small onClick={() => confirmViewing(v.id)}>✓ Potvrdit prohlídku</Btn>
                        <Btn small variant="ghost" onClick={() => cancelViewing(v.id)}>Odmítnout</Btn>
                      </div>
                    )}
                    {v.status === "CONFIRMED" && (
                      <div style={{ fontSize: 12, color: "#059669", fontWeight: 600 }}>✓ Prohlídka potvrzena — kupující byl informován</div>
                    )}
                  </div>
                </div>
              </Card>
    ...
    <div style={{ maxWidth: 1050, margin: "0 auto", padding: "28px 24px" }}>
      ...
      {tab === "viewings" && (() => {
        ...
        if (myViewings.length === 0) return (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>📅</div>
            <div style={{ fontWeight: 600, marginBottom: 8 }}>Zatím žádné prohlídky</div>
            <p style={{ color: "#888", marginBottom: 20 }}>Najděte nemovitost a rezervujte si prohlídku</p>
            <Btn onClick={() => setPage("listings")}>Procházet nabídky →</Btn>
          </div>
        );
        return (
